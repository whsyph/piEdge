...
const i18n = {
  id: {
    ...
    about_sub:
      "Informasi detail mengenai perangkat lunak kontrol pemutar video piEdge",
    about_desc_1:
      "<strong>piEdge CMS Signage</strong> adalah sistem manajemen konten (CMS) signage multimedia berbasis web yang dikembangkan khusus untuk mengelola pemutaran video secara mandiri maupun terpusat pada Raspberry Pi 4.",
    about_desc_2:
      "Software ini mengintegrasikan server web multi-thread berdaya rendah dengan antarmuka pemrograman video MPV melalui soket IPC Unix, memungkinkan kontrol pemutaran video berkinerja tinggi langsung to port hardware HDMI (HDMI-A-1 dan HDMI-A-2) secara mulus, efisien, dan tanpa hambatan.",
    ...
    toast_device_offline_preview:
      "Perangkat offline, tidak bisa mengambil preview",
    toast_capturing_screenshot:
      "Mengambil tangkapan layar baru untuk Layar {screen}...",
    ...
    library_sub_desc:
      "Unggah video baru dan tentukan urutan daftar putar pada tiap layar",
    ...
    confirm_delete_video:
      'Apakah Anda yakin ingin menghapus "{filename}" dari Layar {screen}?',
    ...
    toast_audio_channel_changed:
      "Channel Audio Layar {screen} diubah ke {channel}",
    ...
    config_sub:
      "Sesuaikan tema warna preset, bahasa antarmuka, dan jalur koneksi jaringan",
    ...
    confirm_change_network:
      "PERINGATAN: Mengubah jalur jaringan akan mematikan antarmuka yang tidak terpilih. Jika Anda terhubung menggunakan antarmuka tersebut, koneksi Anda ke CMS ini akan terputus! Apakah Anda yakin ingin melanjutkan?",
    nav_schedule: "📅 Penjadwalan",
    schedule_header: "Jadwal Operasional",
    schedule_sub:
      "Atur jam hidup/mati otomatis dan jadwal pergantian playlist layar",
    ...
    confirm_shutdown:
      "PERINGATAN: Apakah Anda yakin ingin mematikan (Shutdown) Raspberry Pi ini? Pi akan mati dan Anda harus mencabut-colok kabel daya atau menekan tombol fisik secara manual untuk menyalakannya kembali!",
    nav_playlists: "📋 Pengelola Playlist",
    media_files_list_title: "Daftar Berkas Aset Video",
    media_files_list_sub:
      "Semua video yang tersimpan di Raspberry Pi. Video-video ini dapat dimasukkan ke playlist mana saja.",
    playlist_list_title: "Daftar Playlist",
    playlist_compose_sub:
      "Pilih video di bawah untuk dimasukkan ke playlist, dan gunakan 🔼 / 🔽 untuk mengatur urutan putar.",
    assigned_playlist_header: "Playlist Aktif",
    toast_playlist_assigned:
      "Playlist {playlist} berhasil dipasang ke Layar {screen}!",
    confirm_delete_playlist:
      'Apakah Anda yakin ingin menghapus playlist "{playlistName}"?',
    confirm_delete_video_global:
      'Apakah Anda yakin ingin menghapus "{filename}"? Berkas ini akan dihapus permanen dari sistem dan dari semua playlist.',
  },
  en: {
    brand_name: "caPiBarra",
    brand_sub: "Raspberry Pi Video CMS",
    ...
    about_sub:
      "Detailed information about the piEdge video player control software",
    about_desc_1:
      "<strong>piEdge CMS Signage</strong> is a web-based multimedia signage content management system (CMS) specifically developed to manage video playback independently or centrally on Raspberry Pi 4.",
    about_desc_2:
      "This software integrates a low-power multi-threaded web server with the MPV video programming interface via Unix IPC sockets, enabling high-performance video playback control directly to the hardware HDMI ports (HDMI-A-1 and HDMI-A-2) seamlessly, efficiently, and without lag.",
    ...
    toast_capturing_screenshot:
      "Capturing new screenshot for Screen {screen}...",
    ...
    library_sub_desc:
      "Upload new videos and configure playlist ordering for each screen",
    playlist_tab_1: "Screen 1 Playlist",
    playlist_tab_2: "Screen 2 Playlist",
    settings_header: "System & Device Configuration",
    settings_sub:
      "Manage Raspberry Pi IP addresses and view performance diagnostics",
    ...
    confirm_delete_video:
      'Are you sure you want to delete "{filename}" from Screen {screen}?',
    ...
    toast_audio_channel_changed:
      "Screen {screen} audio channel changed to {channel}",
    ...
    config_sub:
      "Customize color theme presets, interface language, and network connection path",
    ...
    confirm_change_network:
      "WARNING: Changing the network path will disable the unselected interface. If you are connected using that interface, your connection to this CMS will be lost! Are you sure you want to proceed?",
    nav_schedule: "📅 Scheduling",
    schedule_header: "Operational Schedule",
    schedule_sub:
      "Configure automatic power on/off and screen playlist changes",
    ...
    btn_shutdown: "🔌 Shutdown Pi",
    confirm_shutdown:
      "WARNING: Are you sure you want to shutdown this Raspberry Pi? It will power off, and you must manually reconnect power to turn it back on!",
    nav_playlists: "📋 Playlist Manager",
    media_files_list_title: "Video Asset Files List",
    media_files_list_sub:
      "All videos saved on the Raspberry Pi. These videos can be added to any playlist.",
    playlist_list_title: "Playlist List",
    playlist_compose_sub:
      "Select videos below to include in the playlist, and use 🔼 / 🔽 to adjust their playback order.",
    assigned_playlist_header: "Active Playlist",
    toast_playlist_assigned:
      "Playlist {playlist} successfully assigned to Screen {screen}!",
    confirm_delete_playlist:
      'Are you sure you want to delete playlist "{playlistName}"?',
    confirm_delete_video_global:
      'Are you sure you want to delete "{filename}"? This file will be permanently deleted from the system and all playlists.',
  },
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  loadDevices().then(() => {
    if (devices.length > 0) {
      ...
      const defaultDev =
        devices.find((d) => d.id === "pi-1-local") || devices[0];
...
async function loadDevices() {
  try {
    ...
    console.warn(
      "Gagal memuat devices.json dari server, memakai fallback local storage",
      e,
    );
  }
  ...
  const deletedIds = JSON.parse(
    localStorage.getItem("museum_signage_deleted_devices") || "[]",
  );
...
async function fetchActiveDeviceStatus() {
  ...
  try {
    ...
    document.getElementById("s1-status-badge").textContent = t(
      "status_offline",
      "Offline",
    );
    document.getElementById("s2-status-badge").className = "badge badge-error";
    document.getElementById("s2-status-badge").textContent = t(
      "status_offline",
      "Offline",
    );
  }
...
function updateScreenControlUI(screenNum, data, audioData) {
  ...
  const routeSelect = document.getElementById(
    `s${screenNum}-select-audio-route`,
  );
  const channelSelect = document.getElementById(
    `s${screenNum}-select-audio-channel`,
  );
  ...
  if (data.service_active) {
    ...
    statusBadge.textContent = data.mpv_connected
      ? t("status_playing", "Playing")
      : t("status_active_no_mpv", "Active (No MPV)");
  } else {
  ...
  // Playing title
  titleEl.textContent =
    data.playing_file !== "None"
      ? data.playing_file
      : t("no_media", "Daftar putar kosong / Berhenti");
...
function updateDiagnosticsUI(sys) {
  ...
  document.getElementById("diag-ram").textContent =
    `${sys.ram.used_mb} / ${sys.ram.total_mb} MB (${sys.ram.percent}%)`;
...
async function controlDevice(screen, action) {
  try {
    ...
    if (data.success) {
      ...
      showToast(
        t("toast_command_fail", "Gagal: {error}").replace(
          "{error}",
          data.error,
        ),
        true,
    ...
    showToast(
      t("toast_command_error", "Error mengirim perintah kontrol"),
      true,
    );
  }
}
...
async function updateOverviewGrid() {
  ...
  results.forEach((res) => {
    ...
    if (res.online && res.data) {
      screen1Name =
        res.data.screen1.playing_file !== "None"
          ? res.data.screen1.playing_file
          : "Berhenti";
      screen2Name =
        res.data.screen2.playing_file !== "None"
          ? res.data.screen2.playing_file
          : "Berhenti";
...
function updateMediaLibraryUI(allFiles, playlist1, playlist2) {
  ...
  if (playlist1)
    playlist1.forEach((f) => {
      if (f.size_mb) sizes[f.name] = f.size_mb;
    });
  if (playlist2)
    playlist2.forEach((f) => {
      if (f.size_mb) sizes[f.name] = f.size_mb;
  ...
  allFiles.forEach((fname) => {
    ...
    const s1Settings =
      (audioConfig.screen1 && audioConfig.screen1.clip_settings) || {};
    const isClipMuted = s1Settings[fname] ? s1Settings[fname].mute : false;

    const muteBadgeClass = isClipMuted
      ? "badge badge-error"
      : "badge badge-success";
    const muteBadgeText = isClipMuted
      ? t("badge_muted", "🔇 Muted")
      : t("badge_sound", "🔊 Sound");
...
async function toggleClipMuteGlobal(filename) {
  ...
  try {
    ...
    const msg = newMute
      ? t("toast_clip_muted", 'Audio klip "{filename}" Dimatikan').replace(
          "{filename}",
          filename,
        )
      : t("toast_clip_unmuted", 'Audio klip "{filename}" Dihidupkan').replace(
          "{filename}",
          filename,
        );
    showToast(msg);
    ...
    showToast(
      t("toast_command_error", "Gagal mengubah status audio klip"),
      true,
    );
  }
}
...
function setupUploadZone() {
  ...
  function handleFileUpload(file) {
    ...
    xhr.addEventListener("error", () => {
      showToast(
        t("toast_upload_error", "Koneksi gagal saat mengunggah file"),
        true,
      );
...
function setupSettingsForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    form.reset();
    showToast(
      t("toast_device_added", 'Perangkat "{name}" ditambahkan').replace(
        "{name}",
        name,
      ),
    );
...
function deleteRegisteredDevice(id) {
  ...
  const msg = t(
    ...
    'Hapus perangkat "{name}" dari daftar kontrol?',
  ...
  if (!deletedIds.includes(id)) {
    ...
    localStorage.setItem(
      "museum_signage_deleted_devices",
...
async function toggleGlobalMute(screen) {
  ...
  const currentMute = audioConfig[screenKey]
    ? audioConfig[screenKey].global_mute
    : true;
  ...
  try {
    ...
    if (data.success) {
      const msg = !currentMute
        ? t("toast_audio_muted", "Audio Layar {screen} Dimatikan").replace(
            "{screen}",
            screen,
          )
        : t("toast_audio_unmuted", "Audio Layar {screen} Dihidupkan").replace(
            "{screen}",
            screen,
          );
      showToast(msg);
...
async function changeAudioRoute(screen) {
  ...
  try {
    ...
    if (data.success) {
      showToast(
        t(
          "toast_audio_route_changed",
          "Output Audio Layar {screen} dialihkan ke {route}",
        )
          .replace("{screen}", screen)
...
let playlistsData = {
  playlists: { Default: [] },
  assignments: { screen1: "Default", screen2: "Default" },
};
  ...
  try {
    ...
    if (pRes.ok) {
      ...
      if (!playlistsData.assignments)
        playlistsData.assignments = { screen1: "Default", screen2: "Default" };
...
function updateControlScreenPlaylistDropdowns() {
  ...
  const s1Active =
    (playlistsData.assignments && playlistsData.assignments.screen1) ||
    "Default";
  const s2Active =
    (playlistsData.assignments && playlistsData.assignments.screen2) ||
    "Default";
...
async function saveAllPlaylists() {
  try {
    ...
    showToast(
      `Gagal menyimpan playlist: ${data.error || "Unknown error"}`,
      true,
    );
    return false;
  } catch (e) {
    console.error("[Playlist Save]", e);
    showToast(`Error saving playlists: ${e.message}`, true);
    return false;
  }
}
...
function renderPlaylistsUI() {
  ...
  Object.keys(playlists).forEach((pName) => {
    ...
    if (pName !== "Default") {
      ...
      delBtn.title = t("confirm_delete_playlist", "Hapus playlist ini").replace(
        "{playlistName}",
        pName,
      );
  ...
  const allFiles =
    (window.lastFetchedStatus &&
      window.lastFetchedStatus.screen1 &&
      window.lastFetchedStatus.screen1.all_files) ||
    [];
  syncPlaylistCompositionUI(allFiles);
}
...
function deletePlaylist(pName) {
  ...
  if (playlistsData.playlists && playlistsData.playlists[pName]) {
    ...
    if (playlistsData.assignments) {
      if (playlistsData.assignments.screen1 === pName)
        playlistsData.assignments.screen1 = "Default";
      if (playlistsData.assignments.screen2 === pName)
        playlistsData.assignments.screen2 = "Default";
    }
...
async function savePlaylistComposition() {
  const listEl = document.getElementById("playlist-composition-items");
  if (!listEl) return;
  ...
  const success = await saveAllPlaylists();
...
function updateScheduleUI() {
  ...
  schedulesData.events.forEach((ev) => {
    ...
    const actionStr =
      ev.type === "power"
        ? `Power ${ev.action.toUpperCase()}`
        : `Load Playlist`;
    const targetStr =
      ev.type === "power"
        ? "All System"
        : `Screen ${ev.screen} (${ev.playlist_name})`;
...
window.saveTimezone = async function () {
  schedulesData.timezone_offset = parseInt(
    document.getElementById("tz-select").value,
  );
...
window.saveNewSchedule = async function () {
  ...
  if (days.length === 0) {
    alert("Pilih minimal 1 hari pelaksanaan");
    return;
  }

  // Conflict Check
  const isConflict = schedulesData.events.some(
    (ev) => ev.time === time && ev.days.some((d) => days.includes(d)),
  );
  if (isConflict) {
    if (
      !confirm(
        t(
          "toast_conflict",
          "Konflik: Jadwal sudah ada di waktu tersebut! Lanjutkan?",
        ),
      )
    )
      return;
...
window.shutdownSystem = async function () {
  ...
  try {
    const res = await fetch(getApiUrl("/api/system/shutdown"), {
      method: "POST",
    });
    if (res.ok) {
      showToast(
        "Perintah Shutdown berhasil dikirim. Alat akan mati dalam 1 detik.",
      );
    } else {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (ethStatusEl && data.interfaces.eth) {
        const ethInfo = data.interfaces.eth
          .map((i) => `${i.name}: ${i.ip} (${i.status.toUpperCase()})`)
          .join("<br>");
        ethStatusEl.innerHTML = ethInfo || "None";
      }
      if (wlanStatusEl && data.interfaces.wlan) {
        const wlanInfo = data.interfaces.wlan
          .map((i) => `${i.name}: ${i.ip} (${i.status.toUpperCase()})`)
          .join("<br>");
  ...
  try {
    ...
    if (data.success) {
      showToast(
        t(
          "toast_network_saved",
          "Jalur jaringan berhasil diubah ke {interface}!",
        ).replace("{interface}", selected.toUpperCase()),
      );
